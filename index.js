...
class UpdateNotifier {
	...
	async checkNpm() {
		...
		const latestVersionOptions = registryUrl ? {version: distTag, registryUrl} : {version: distTag};
		const latest = await latestVersion()(this.packageName, latestVersionOptions);
	...
	notify(options) {
		...
		let installCommand;
		const {registryUrl} = this.options
		if (registryUrl) {
			installCommand = `npm i ${this.packageName} --registry=${registryUrl}`;
		} else {
